...
import ProjectSystemSecurityPlan from "./ProjectSystemSecurityPlan";
...
test("renders the LoadingIcon when waiting for data, then renders the pageTemplate when page data is successfully returned", async () => {
  ...
  render(
    <MemoryRouter
      initialEntries={[`/projects/${pageData.id}/system-security-plan`]}
    >
      <GlobalStateProvider>
        <Routes>
          <Route
            path="projects/:id/system-security-plan"
...
test("renders the ErrorMessage when page data is NOT successfully returned", async () => {
  ...
  render(
    <MemoryRouter
      initialEntries={[`/projects/${nonExistentId}/system-security-plan`]}
    >
      <GlobalStateProvider>
        <Routes>
          <Route
